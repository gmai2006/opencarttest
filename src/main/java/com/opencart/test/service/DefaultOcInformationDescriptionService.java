/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcInformationDescriptionDao;
import com.opencart.test.entity.OcInformationDescription;
import com.opencart.test.entity.OcInformationDescriptionId;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcInformationDescriptionService")
public class DefaultOcInformationDescriptionService implements OcInformationDescriptionService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcInformationDescriptionDao dao;

    @Inject
    @Named("DefaultOcInformationDescriptionDao")
    public DefaultOcInformationDescriptionService(final OcInformationDescriptionDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcInformationDescription find(OcInformationDescriptionId id) {
        final OcInformationDescription result = dao.find(id);
        logger.info("find(OcInformationDescription) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcInformationDescription> select(int maxResult) {
        final List<OcInformationDescription> result = dao.select(maxResult);
        logger.info("select(OcInformationDescription) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcInformationDescription> selectAll() {
        final List<OcInformationDescription> results = dao.selectAll();
        logger.info("selectAll(OcInformationDescription) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcInformationDescription create(OcInformationDescription bean) {
        requireNonNull(bean);
        logger.info("create(OcInformationDescription={}) - entered bean ");

        final OcInformationDescription result = dao.create(bean);

        logger.info("create(OcInformationDescription) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcInformationDescription update(OcInformationDescription bean) {
        requireNonNull(bean);
        logger.info("update(OcInformationDescription={}) - entered bean ");

        final OcInformationDescription result = dao.update(bean);

        logger.info("update(OcInformationDescription) - exited - return value={} result ");
        return result;
    }
}
