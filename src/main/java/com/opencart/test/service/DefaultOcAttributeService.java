/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcAttributeDao;
import com.opencart.test.entity.OcAttribute;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcAttributeService")
public class DefaultOcAttributeService implements OcAttributeService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcAttributeDao dao;

    @Inject
    @Named("DefaultOcAttributeDao")
    public DefaultOcAttributeService(final OcAttributeDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcAttribute find(java.lang.Integer id) {

        final OcAttribute result = dao.find(id);
        logger.info("find(OcAttribute) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcAttribute> select(int maxResult) {
        final List<OcAttribute> result = dao.select(maxResult);
        logger.info("select(OcAttribute) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcAttribute> selectAll() {
        final List<OcAttribute> results = dao.selectAll();
        logger.info("selectAll(OcAttribute) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcAttribute create(OcAttribute bean) {
        requireNonNull(bean);
        logger.info("create(OcAttribute={}) - entered bean ");

        final OcAttribute result = dao.create(bean);

        logger.info("create(OcAttribute) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcAttribute update(OcAttribute bean) {
        requireNonNull(bean);
        logger.info("update(OcAttribute={}) - entered bean ");

        final OcAttribute result = dao.update(bean);

        logger.info("update(OcAttribute) - exited - return value={} result ");
        return result;
    }
}
