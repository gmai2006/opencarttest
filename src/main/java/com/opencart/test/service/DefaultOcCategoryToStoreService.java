/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcCategoryToStoreDao;
import com.opencart.test.entity.OcCategoryToStore;
import com.opencart.test.entity.OcCategoryToStoreId;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcCategoryToStoreService")
public class DefaultOcCategoryToStoreService implements OcCategoryToStoreService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcCategoryToStoreDao dao;

    @Inject
    @Named("DefaultOcCategoryToStoreDao")
    public DefaultOcCategoryToStoreService(final OcCategoryToStoreDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcCategoryToStore find(OcCategoryToStoreId id) {
        final OcCategoryToStore result = dao.find(id);
        logger.info("find(OcCategoryToStore) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcCategoryToStore> select(int maxResult) {
        final List<OcCategoryToStore> result = dao.select(maxResult);
        logger.info("select(OcCategoryToStore) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcCategoryToStore> selectAll() {
        final List<OcCategoryToStore> results = dao.selectAll();
        logger.info("selectAll(OcCategoryToStore) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcCategoryToStore create(OcCategoryToStore bean) {
        requireNonNull(bean);
        logger.info("create(OcCategoryToStore={}) - entered bean ");

        final OcCategoryToStore result = dao.create(bean);

        logger.info("create(OcCategoryToStore) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcCategoryToStore update(OcCategoryToStore bean) {
        requireNonNull(bean);
        logger.info("update(OcCategoryToStore={}) - entered bean ");

        final OcCategoryToStore result = dao.update(bean);

        logger.info("update(OcCategoryToStore) - exited - return value={} result ");
        return result;
    }
}
