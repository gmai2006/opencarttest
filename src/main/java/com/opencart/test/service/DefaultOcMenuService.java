/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcMenuDao;
import com.opencart.test.entity.OcMenu;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcMenuService")
public class DefaultOcMenuService implements OcMenuService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcMenuDao dao;

    @Inject
    @Named("DefaultOcMenuDao")
    public DefaultOcMenuService(final OcMenuDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcMenu find(java.lang.Integer id) {

        final OcMenu result = dao.find(id);
        logger.info("find(OcMenu) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcMenu> select(int maxResult) {
        final List<OcMenu> result = dao.select(maxResult);
        logger.info("select(OcMenu) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcMenu> selectAll() {
        final List<OcMenu> results = dao.selectAll();
        logger.info("selectAll(OcMenu) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcMenu create(OcMenu bean) {
        requireNonNull(bean);
        logger.info("create(OcMenu={}) - entered bean ");

        final OcMenu result = dao.create(bean);

        logger.info("create(OcMenu) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcMenu update(OcMenu bean) {
        requireNonNull(bean);
        logger.info("update(OcMenu={}) - entered bean ");

        final OcMenu result = dao.update(bean);

        logger.info("update(OcMenu) - exited - return value={} result ");
        return result;
    }
}
