/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcFilterDao;
import com.opencart.test.entity.OcFilter;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcFilterService")
public class DefaultOcFilterService implements OcFilterService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcFilterDao dao;

    @Inject
    @Named("DefaultOcFilterDao")
    public DefaultOcFilterService(final OcFilterDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcFilter find(java.lang.Integer id) {

        final OcFilter result = dao.find(id);
        logger.info("find(OcFilter) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcFilter> select(int maxResult) {
        final List<OcFilter> result = dao.select(maxResult);
        logger.info("select(OcFilter) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcFilter> selectAll() {
        final List<OcFilter> results = dao.selectAll();
        logger.info("selectAll(OcFilter) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcFilter create(OcFilter bean) {
        requireNonNull(bean);
        logger.info("create(OcFilter={}) - entered bean ");

        final OcFilter result = dao.create(bean);

        logger.info("create(OcFilter) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcFilter update(OcFilter bean) {
        requireNonNull(bean);
        logger.info("update(OcFilter={}) - entered bean ");

        final OcFilter result = dao.update(bean);

        logger.info("update(OcFilter) - exited - return value={} result ");
        return result;
    }
}
