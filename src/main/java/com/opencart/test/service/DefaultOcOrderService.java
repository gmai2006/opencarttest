/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcOrderDao;
import com.opencart.test.entity.OcOrder;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcOrderService")
public class DefaultOcOrderService implements OcOrderService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcOrderDao dao;

    @Inject
    @Named("DefaultOcOrderDao")
    public DefaultOcOrderService(final OcOrderDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcOrder find(java.lang.Integer id) {

        final OcOrder result = dao.find(id);
        logger.info("find(OcOrder) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcOrder> select(int maxResult) {
        final List<OcOrder> result = dao.select(maxResult);
        logger.info("select(OcOrder) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcOrder> selectAll() {
        final List<OcOrder> results = dao.selectAll();
        logger.info("selectAll(OcOrder) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcOrder create(OcOrder bean) {
        requireNonNull(bean);
        logger.info("create(OcOrder={}) - entered bean ");

        final OcOrder result = dao.create(bean);

        logger.info("create(OcOrder) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcOrder update(OcOrder bean) {
        requireNonNull(bean);
        logger.info("update(OcOrder={}) - entered bean ");

        final OcOrder result = dao.update(bean);

        logger.info("update(OcOrder) - exited - return value={} result ");
        return result;
    }
}
