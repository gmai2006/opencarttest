/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcCustomerActivityDao;
import com.opencart.test.entity.OcCustomerActivity;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcCustomerActivityService")
public class DefaultOcCustomerActivityService implements OcCustomerActivityService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcCustomerActivityDao dao;

    @Inject
    @Named("DefaultOcCustomerActivityDao")
    public DefaultOcCustomerActivityService(final OcCustomerActivityDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcCustomerActivity find(java.lang.Integer id) {

        final OcCustomerActivity result = dao.find(id);
        logger.info("find(OcCustomerActivity) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcCustomerActivity> select(int maxResult) {
        final List<OcCustomerActivity> result = dao.select(maxResult);
        logger.info("select(OcCustomerActivity) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcCustomerActivity> selectAll() {
        final List<OcCustomerActivity> results = dao.selectAll();
        logger.info("selectAll(OcCustomerActivity) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcCustomerActivity create(OcCustomerActivity bean) {
        requireNonNull(bean);
        logger.info("create(OcCustomerActivity={}) - entered bean ");

        final OcCustomerActivity result = dao.create(bean);

        logger.info("create(OcCustomerActivity) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcCustomerActivity update(OcCustomerActivity bean) {
        requireNonNull(bean);
        logger.info("update(OcCustomerActivity={}) - entered bean ");

        final OcCustomerActivity result = dao.update(bean);

        logger.info("update(OcCustomerActivity) - exited - return value={} result ");
        return result;
    }
}
