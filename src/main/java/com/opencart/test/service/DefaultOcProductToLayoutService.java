/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcProductToLayoutDao;
import com.opencart.test.entity.OcProductToLayout;
import com.opencart.test.entity.OcProductToLayoutId;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcProductToLayoutService")
public class DefaultOcProductToLayoutService implements OcProductToLayoutService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcProductToLayoutDao dao;

    @Inject
    @Named("DefaultOcProductToLayoutDao")
    public DefaultOcProductToLayoutService(final OcProductToLayoutDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcProductToLayout find(OcProductToLayoutId id) {
        final OcProductToLayout result = dao.find(id);
        logger.info("find(OcProductToLayout) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcProductToLayout> select(int maxResult) {
        final List<OcProductToLayout> result = dao.select(maxResult);
        logger.info("select(OcProductToLayout) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcProductToLayout> selectAll() {
        final List<OcProductToLayout> results = dao.selectAll();
        logger.info("selectAll(OcProductToLayout) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcProductToLayout create(OcProductToLayout bean) {
        requireNonNull(bean);
        logger.info("create(OcProductToLayout={}) - entered bean ");

        final OcProductToLayout result = dao.create(bean);

        logger.info("create(OcProductToLayout) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcProductToLayout update(OcProductToLayout bean) {
        requireNonNull(bean);
        logger.info("update(OcProductToLayout={}) - entered bean ");

        final OcProductToLayout result = dao.update(bean);

        logger.info("update(OcProductToLayout) - exited - return value={} result ");
        return result;
    }
}
