/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcCouponProductDao;
import com.opencart.test.entity.OcCouponProduct;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcCouponProductService")
public class DefaultOcCouponProductService implements OcCouponProductService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcCouponProductDao dao;

    @Inject
    @Named("DefaultOcCouponProductDao")
    public DefaultOcCouponProductService(final OcCouponProductDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcCouponProduct find(java.lang.Integer id) {

        final OcCouponProduct result = dao.find(id);
        logger.info("find(OcCouponProduct) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcCouponProduct> select(int maxResult) {
        final List<OcCouponProduct> result = dao.select(maxResult);
        logger.info("select(OcCouponProduct) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcCouponProduct> selectAll() {
        final List<OcCouponProduct> results = dao.selectAll();
        logger.info("selectAll(OcCouponProduct) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcCouponProduct create(OcCouponProduct bean) {
        requireNonNull(bean);
        logger.info("create(OcCouponProduct={}) - entered bean ");

        final OcCouponProduct result = dao.create(bean);

        logger.info("create(OcCouponProduct) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcCouponProduct update(OcCouponProduct bean) {
        requireNonNull(bean);
        logger.info("update(OcCouponProduct={}) - entered bean ");

        final OcCouponProduct result = dao.update(bean);

        logger.info("update(OcCouponProduct) - exited - return value={} result ");
        return result;
    }
}
