/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcOptionValueDescriptionDao;
import com.opencart.test.entity.OcOptionValueDescription;
import com.opencart.test.entity.OcOptionValueDescriptionId;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcOptionValueDescriptionService")
public class DefaultOcOptionValueDescriptionService implements OcOptionValueDescriptionService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcOptionValueDescriptionDao dao;

    @Inject
    @Named("DefaultOcOptionValueDescriptionDao")
    public DefaultOcOptionValueDescriptionService(final OcOptionValueDescriptionDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcOptionValueDescription find(OcOptionValueDescriptionId id) {
        final OcOptionValueDescription result = dao.find(id);
        logger.info("find(OcOptionValueDescription) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcOptionValueDescription> select(int maxResult) {
        final List<OcOptionValueDescription> result = dao.select(maxResult);
        logger.info("select(OcOptionValueDescription) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcOptionValueDescription> selectAll() {
        final List<OcOptionValueDescription> results = dao.selectAll();
        logger.info("selectAll(OcOptionValueDescription) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcOptionValueDescription create(OcOptionValueDescription bean) {
        requireNonNull(bean);
        logger.info("create(OcOptionValueDescription={}) - entered bean ");

        final OcOptionValueDescription result = dao.create(bean);

        logger.info("create(OcOptionValueDescription) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcOptionValueDescription update(OcOptionValueDescription bean) {
        requireNonNull(bean);
        logger.info("update(OcOptionValueDescription={}) - entered bean ");

        final OcOptionValueDescription result = dao.update(bean);

        logger.info("update(OcOptionValueDescription) - exited - return value={} result ");
        return result;
    }
}
