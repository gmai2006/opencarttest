/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcLengthClassDescriptionDao;
import com.opencart.test.entity.OcLengthClassDescription;
import com.opencart.test.entity.OcLengthClassDescriptionId;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcLengthClassDescriptionService")
public class DefaultOcLengthClassDescriptionService implements OcLengthClassDescriptionService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcLengthClassDescriptionDao dao;

    @Inject
    @Named("DefaultOcLengthClassDescriptionDao")
    public DefaultOcLengthClassDescriptionService(final OcLengthClassDescriptionDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcLengthClassDescription find(OcLengthClassDescriptionId id) {
        final OcLengthClassDescription result = dao.find(id);
        logger.info("find(OcLengthClassDescription) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcLengthClassDescription> select(int maxResult) {
        final List<OcLengthClassDescription> result = dao.select(maxResult);
        logger.info("select(OcLengthClassDescription) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcLengthClassDescription> selectAll() {
        final List<OcLengthClassDescription> results = dao.selectAll();
        logger.info("selectAll(OcLengthClassDescription) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcLengthClassDescription create(OcLengthClassDescription bean) {
        requireNonNull(bean);
        logger.info("create(OcLengthClassDescription={}) - entered bean ");

        final OcLengthClassDescription result = dao.create(bean);

        logger.info("create(OcLengthClassDescription) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcLengthClassDescription update(OcLengthClassDescription bean) {
        requireNonNull(bean);
        logger.info("update(OcLengthClassDescription={}) - entered bean ");

        final OcLengthClassDescription result = dao.update(bean);

        logger.info("update(OcLengthClassDescription) - exited - return value={} result ");
        return result;
    }
}
