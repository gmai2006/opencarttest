/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcReturnDao;
import com.opencart.test.entity.OcReturn;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcReturnService")
public class DefaultOcReturnService implements OcReturnService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcReturnDao dao;

    @Inject
    @Named("DefaultOcReturnDao")
    public DefaultOcReturnService(final OcReturnDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcReturn find(java.lang.Integer id) {

        final OcReturn result = dao.find(id);
        logger.info("find(OcReturn) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcReturn> select(int maxResult) {
        final List<OcReturn> result = dao.select(maxResult);
        logger.info("select(OcReturn) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcReturn> selectAll() {
        final List<OcReturn> results = dao.selectAll();
        logger.info("selectAll(OcReturn) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcReturn create(OcReturn bean) {
        requireNonNull(bean);
        logger.info("create(OcReturn={}) - entered bean ");

        final OcReturn result = dao.create(bean);

        logger.info("create(OcReturn) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcReturn update(OcReturn bean) {
        requireNonNull(bean);
        logger.info("update(OcReturn={}) - entered bean ");

        final OcReturn result = dao.update(bean);

        logger.info("update(OcReturn) - exited - return value={} result ");
        return result;
    }
}
