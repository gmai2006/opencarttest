/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcReturnStatusDao;
import com.opencart.test.entity.OcReturnStatus;
import com.opencart.test.entity.OcReturnStatusId;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcReturnStatusService")
public class DefaultOcReturnStatusService implements OcReturnStatusService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcReturnStatusDao dao;

    @Inject
    @Named("DefaultOcReturnStatusDao")
    public DefaultOcReturnStatusService(final OcReturnStatusDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcReturnStatus find(OcReturnStatusId id) {
        final OcReturnStatus result = dao.find(id);
        logger.info("find(OcReturnStatus) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcReturnStatus> select(int maxResult) {
        final List<OcReturnStatus> result = dao.select(maxResult);
        logger.info("select(OcReturnStatus) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcReturnStatus> selectAll() {
        final List<OcReturnStatus> results = dao.selectAll();
        logger.info("selectAll(OcReturnStatus) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcReturnStatus create(OcReturnStatus bean) {
        requireNonNull(bean);
        logger.info("create(OcReturnStatus={}) - entered bean ");

        final OcReturnStatus result = dao.create(bean);

        logger.info("create(OcReturnStatus) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcReturnStatus update(OcReturnStatus bean) {
        requireNonNull(bean);
        logger.info("update(OcReturnStatus={}) - entered bean ");

        final OcReturnStatus result = dao.update(bean);

        logger.info("update(OcReturnStatus) - exited - return value={} result ");
        return result;
    }
}
