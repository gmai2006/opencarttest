/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcProductAttributeDao;
import com.opencart.test.entity.OcProductAttribute;
import com.opencart.test.entity.OcProductAttributeId;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcProductAttributeService")
public class DefaultOcProductAttributeService implements OcProductAttributeService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcProductAttributeDao dao;

    @Inject
    @Named("DefaultOcProductAttributeDao")
    public DefaultOcProductAttributeService(final OcProductAttributeDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcProductAttribute find(OcProductAttributeId id) {
        final OcProductAttribute result = dao.find(id);
        logger.info("find(OcProductAttribute) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcProductAttribute> select(int maxResult) {
        final List<OcProductAttribute> result = dao.select(maxResult);
        logger.info("select(OcProductAttribute) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcProductAttribute> selectAll() {
        final List<OcProductAttribute> results = dao.selectAll();
        logger.info("selectAll(OcProductAttribute) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcProductAttribute create(OcProductAttribute bean) {
        requireNonNull(bean);
        logger.info("create(OcProductAttribute={}) - entered bean ");

        final OcProductAttribute result = dao.create(bean);

        logger.info("create(OcProductAttribute) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcProductAttribute update(OcProductAttribute bean) {
        requireNonNull(bean);
        logger.info("update(OcProductAttribute={}) - entered bean ");

        final OcProductAttribute result = dao.update(bean);

        logger.info("update(OcProductAttribute) - exited - return value={} result ");
        return result;
    }
}
