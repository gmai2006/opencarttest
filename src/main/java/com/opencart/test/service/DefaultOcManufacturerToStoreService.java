/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcManufacturerToStoreDao;
import com.opencart.test.entity.OcManufacturerToStore;
import com.opencart.test.entity.OcManufacturerToStoreId;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcManufacturerToStoreService")
public class DefaultOcManufacturerToStoreService implements OcManufacturerToStoreService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcManufacturerToStoreDao dao;

    @Inject
    @Named("DefaultOcManufacturerToStoreDao")
    public DefaultOcManufacturerToStoreService(final OcManufacturerToStoreDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcManufacturerToStore find(OcManufacturerToStoreId id) {
        final OcManufacturerToStore result = dao.find(id);
        logger.info("find(OcManufacturerToStore) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcManufacturerToStore> select(int maxResult) {
        final List<OcManufacturerToStore> result = dao.select(maxResult);
        logger.info("select(OcManufacturerToStore) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcManufacturerToStore> selectAll() {
        final List<OcManufacturerToStore> results = dao.selectAll();
        logger.info("selectAll(OcManufacturerToStore) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcManufacturerToStore create(OcManufacturerToStore bean) {
        requireNonNull(bean);
        logger.info("create(OcManufacturerToStore={}) - entered bean ");

        final OcManufacturerToStore result = dao.create(bean);

        logger.info("create(OcManufacturerToStore) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcManufacturerToStore update(OcManufacturerToStore bean) {
        requireNonNull(bean);
        logger.info("update(OcManufacturerToStore={}) - entered bean ");

        final OcManufacturerToStore result = dao.update(bean);

        logger.info("update(OcManufacturerToStore) - exited - return value={} result ");
        return result;
    }
}
