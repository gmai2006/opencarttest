/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcRecurringDao;
import com.opencart.test.entity.OcRecurring;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcRecurringService")
public class DefaultOcRecurringService implements OcRecurringService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcRecurringDao dao;

    @Inject
    @Named("DefaultOcRecurringDao")
    public DefaultOcRecurringService(final OcRecurringDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcRecurring find(java.lang.Integer id) {

        final OcRecurring result = dao.find(id);
        logger.info("find(OcRecurring) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcRecurring> select(int maxResult) {
        final List<OcRecurring> result = dao.select(maxResult);
        logger.info("select(OcRecurring) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcRecurring> selectAll() {
        final List<OcRecurring> results = dao.selectAll();
        logger.info("selectAll(OcRecurring) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcRecurring create(OcRecurring bean) {
        requireNonNull(bean);
        logger.info("create(OcRecurring={}) - entered bean ");

        final OcRecurring result = dao.create(bean);

        logger.info("create(OcRecurring) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcRecurring update(OcRecurring bean) {
        requireNonNull(bean);
        logger.info("update(OcRecurring={}) - entered bean ");

        final OcRecurring result = dao.update(bean);

        logger.info("update(OcRecurring) - exited - return value={} result ");
        return result;
    }
}
