/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcApiIpDao;
import com.opencart.test.entity.OcApiIp;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcApiIpService")
public class DefaultOcApiIpService implements OcApiIpService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcApiIpDao dao;

    @Inject
    @Named("DefaultOcApiIpDao")
    public DefaultOcApiIpService(final OcApiIpDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcApiIp find(java.lang.Integer id) {

        final OcApiIp result = dao.find(id);
        logger.info("find(OcApiIp) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcApiIp> select(int maxResult) {
        final List<OcApiIp> result = dao.select(maxResult);
        logger.info("select(OcApiIp) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcApiIp> selectAll() {
        final List<OcApiIp> results = dao.selectAll();
        logger.info("selectAll(OcApiIp) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcApiIp create(OcApiIp bean) {
        requireNonNull(bean);
        logger.info("create(OcApiIp={}) - entered bean ");

        final OcApiIp result = dao.create(bean);

        logger.info("create(OcApiIp) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcApiIp update(OcApiIp bean) {
        requireNonNull(bean);
        logger.info("update(OcApiIp={}) - entered bean ");

        final OcApiIp result = dao.update(bean);

        logger.info("update(OcApiIp) - exited - return value={} result ");
        return result;
    }
}
