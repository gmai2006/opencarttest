/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcCouponHistoryDao;
import com.opencart.test.entity.OcCouponHistory;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcCouponHistoryService")
public class DefaultOcCouponHistoryService implements OcCouponHistoryService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcCouponHistoryDao dao;

    @Inject
    @Named("DefaultOcCouponHistoryDao")
    public DefaultOcCouponHistoryService(final OcCouponHistoryDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcCouponHistory find(java.lang.Integer id) {

        final OcCouponHistory result = dao.find(id);
        logger.info("find(OcCouponHistory) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcCouponHistory> select(int maxResult) {
        final List<OcCouponHistory> result = dao.select(maxResult);
        logger.info("select(OcCouponHistory) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcCouponHistory> selectAll() {
        final List<OcCouponHistory> results = dao.selectAll();
        logger.info("selectAll(OcCouponHistory) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcCouponHistory create(OcCouponHistory bean) {
        requireNonNull(bean);
        logger.info("create(OcCouponHistory={}) - entered bean ");

        final OcCouponHistory result = dao.create(bean);

        logger.info("create(OcCouponHistory) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcCouponHistory update(OcCouponHistory bean) {
        requireNonNull(bean);
        logger.info("update(OcCouponHistory={}) - entered bean ");

        final OcCouponHistory result = dao.update(bean);

        logger.info("update(OcCouponHistory) - exited - return value={} result ");
        return result;
    }
}
