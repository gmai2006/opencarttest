/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcMenuDescriptionDao;
import com.opencart.test.entity.OcMenuDescription;
import com.opencart.test.entity.OcMenuDescriptionId;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcMenuDescriptionService")
public class DefaultOcMenuDescriptionService implements OcMenuDescriptionService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcMenuDescriptionDao dao;

    @Inject
    @Named("DefaultOcMenuDescriptionDao")
    public DefaultOcMenuDescriptionService(final OcMenuDescriptionDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcMenuDescription find(OcMenuDescriptionId id) {
        final OcMenuDescription result = dao.find(id);
        logger.info("find(OcMenuDescription) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcMenuDescription> select(int maxResult) {
        final List<OcMenuDescription> result = dao.select(maxResult);
        logger.info("select(OcMenuDescription) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcMenuDescription> selectAll() {
        final List<OcMenuDescription> results = dao.selectAll();
        logger.info("selectAll(OcMenuDescription) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcMenuDescription create(OcMenuDescription bean) {
        requireNonNull(bean);
        logger.info("create(OcMenuDescription={}) - entered bean ");

        final OcMenuDescription result = dao.create(bean);

        logger.info("create(OcMenuDescription) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcMenuDescription update(OcMenuDescription bean) {
        requireNonNull(bean);
        logger.info("update(OcMenuDescription={}) - entered bean ");

        final OcMenuDescription result = dao.update(bean);

        logger.info("update(OcMenuDescription) - exited - return value={} result ");
        return result;
    }
}
