/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcProductToDownloadDao;
import com.opencart.test.entity.OcProductToDownload;
import com.opencart.test.entity.OcProductToDownloadId;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcProductToDownloadService")
public class DefaultOcProductToDownloadService implements OcProductToDownloadService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcProductToDownloadDao dao;

    @Inject
    @Named("DefaultOcProductToDownloadDao")
    public DefaultOcProductToDownloadService(final OcProductToDownloadDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcProductToDownload find(OcProductToDownloadId id) {
        final OcProductToDownload result = dao.find(id);
        logger.info("find(OcProductToDownload) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcProductToDownload> select(int maxResult) {
        final List<OcProductToDownload> result = dao.select(maxResult);
        logger.info("select(OcProductToDownload) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcProductToDownload> selectAll() {
        final List<OcProductToDownload> results = dao.selectAll();
        logger.info("selectAll(OcProductToDownload) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcProductToDownload create(OcProductToDownload bean) {
        requireNonNull(bean);
        logger.info("create(OcProductToDownload={}) - entered bean ");

        final OcProductToDownload result = dao.create(bean);

        logger.info("create(OcProductToDownload) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcProductToDownload update(OcProductToDownload bean) {
        requireNonNull(bean);
        logger.info("update(OcProductToDownload={}) - entered bean ");

        final OcProductToDownload result = dao.update(bean);

        logger.info("update(OcProductToDownload) - exited - return value={} result ");
        return result;
    }
}
