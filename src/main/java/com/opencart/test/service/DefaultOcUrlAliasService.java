/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcUrlAliasDao;
import com.opencart.test.entity.OcUrlAlias;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcUrlAliasService")
public class DefaultOcUrlAliasService implements OcUrlAliasService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcUrlAliasDao dao;

    @Inject
    @Named("DefaultOcUrlAliasDao")
    public DefaultOcUrlAliasService(final OcUrlAliasDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcUrlAlias find(java.lang.Integer id) {

        final OcUrlAlias result = dao.find(id);
        logger.info("find(OcUrlAlias) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcUrlAlias> select(int maxResult) {
        final List<OcUrlAlias> result = dao.select(maxResult);
        logger.info("select(OcUrlAlias) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcUrlAlias> selectAll() {
        final List<OcUrlAlias> results = dao.selectAll();
        logger.info("selectAll(OcUrlAlias) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcUrlAlias create(OcUrlAlias bean) {
        requireNonNull(bean);
        logger.info("create(OcUrlAlias={}) - entered bean ");

        final OcUrlAlias result = dao.create(bean);

        logger.info("create(OcUrlAlias) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcUrlAlias update(OcUrlAlias bean) {
        requireNonNull(bean);
        logger.info("update(OcUrlAlias={}) - entered bean ");

        final OcUrlAlias result = dao.update(bean);

        logger.info("update(OcUrlAlias) - exited - return value={} result ");
        return result;
    }
}
