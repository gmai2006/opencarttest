/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcRecurringDescriptionDao;
import com.opencart.test.entity.OcRecurringDescription;
import com.opencart.test.entity.OcRecurringDescriptionId;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcRecurringDescriptionService")
public class DefaultOcRecurringDescriptionService implements OcRecurringDescriptionService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcRecurringDescriptionDao dao;

    @Inject
    @Named("DefaultOcRecurringDescriptionDao")
    public DefaultOcRecurringDescriptionService(final OcRecurringDescriptionDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcRecurringDescription find(OcRecurringDescriptionId id) {
        final OcRecurringDescription result = dao.find(id);
        logger.info("find(OcRecurringDescription) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcRecurringDescription> select(int maxResult) {
        final List<OcRecurringDescription> result = dao.select(maxResult);
        logger.info("select(OcRecurringDescription) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcRecurringDescription> selectAll() {
        final List<OcRecurringDescription> results = dao.selectAll();
        logger.info("selectAll(OcRecurringDescription) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcRecurringDescription create(OcRecurringDescription bean) {
        requireNonNull(bean);
        logger.info("create(OcRecurringDescription={}) - entered bean ");

        final OcRecurringDescription result = dao.create(bean);

        logger.info("create(OcRecurringDescription) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcRecurringDescription update(OcRecurringDescription bean) {
        requireNonNull(bean);
        logger.info("update(OcRecurringDescription={}) - entered bean ");

        final OcRecurringDescription result = dao.update(bean);

        logger.info("update(OcRecurringDescription) - exited - return value={} result ");
        return result;
    }
}
