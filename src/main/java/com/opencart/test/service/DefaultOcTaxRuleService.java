/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcTaxRuleDao;
import com.opencart.test.entity.OcTaxRule;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcTaxRuleService")
public class DefaultOcTaxRuleService implements OcTaxRuleService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcTaxRuleDao dao;

    @Inject
    @Named("DefaultOcTaxRuleDao")
    public DefaultOcTaxRuleService(final OcTaxRuleDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcTaxRule find(java.lang.Integer id) {

        final OcTaxRule result = dao.find(id);
        logger.info("find(OcTaxRule) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcTaxRule> select(int maxResult) {
        final List<OcTaxRule> result = dao.select(maxResult);
        logger.info("select(OcTaxRule) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcTaxRule> selectAll() {
        final List<OcTaxRule> results = dao.selectAll();
        logger.info("selectAll(OcTaxRule) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcTaxRule create(OcTaxRule bean) {
        requireNonNull(bean);
        logger.info("create(OcTaxRule={}) - entered bean ");

        final OcTaxRule result = dao.create(bean);

        logger.info("create(OcTaxRule) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcTaxRule update(OcTaxRule bean) {
        requireNonNull(bean);
        logger.info("update(OcTaxRule={}) - entered bean ");

        final OcTaxRule result = dao.update(bean);

        logger.info("update(OcTaxRule) - exited - return value={} result ");
        return result;
    }
}
