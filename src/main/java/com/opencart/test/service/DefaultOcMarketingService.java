/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcMarketingDao;
import com.opencart.test.entity.OcMarketing;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcMarketingService")
public class DefaultOcMarketingService implements OcMarketingService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcMarketingDao dao;

    @Inject
    @Named("DefaultOcMarketingDao")
    public DefaultOcMarketingService(final OcMarketingDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcMarketing find(java.lang.Integer id) {

        final OcMarketing result = dao.find(id);
        logger.info("find(OcMarketing) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcMarketing> select(int maxResult) {
        final List<OcMarketing> result = dao.select(maxResult);
        logger.info("select(OcMarketing) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcMarketing> selectAll() {
        final List<OcMarketing> results = dao.selectAll();
        logger.info("selectAll(OcMarketing) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcMarketing create(OcMarketing bean) {
        requireNonNull(bean);
        logger.info("create(OcMarketing={}) - entered bean ");

        final OcMarketing result = dao.create(bean);

        logger.info("create(OcMarketing) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcMarketing update(OcMarketing bean) {
        requireNonNull(bean);
        logger.info("update(OcMarketing={}) - entered bean ");

        final OcMarketing result = dao.update(bean);

        logger.info("update(OcMarketing) - exited - return value={} result ");
        return result;
    }
}
