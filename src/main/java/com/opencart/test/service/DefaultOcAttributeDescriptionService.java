/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcAttributeDescriptionDao;
import com.opencart.test.entity.OcAttributeDescription;
import com.opencart.test.entity.OcAttributeDescriptionId;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcAttributeDescriptionService")
public class DefaultOcAttributeDescriptionService implements OcAttributeDescriptionService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcAttributeDescriptionDao dao;

    @Inject
    @Named("DefaultOcAttributeDescriptionDao")
    public DefaultOcAttributeDescriptionService(final OcAttributeDescriptionDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcAttributeDescription find(OcAttributeDescriptionId id) {
        final OcAttributeDescription result = dao.find(id);
        logger.info("find(OcAttributeDescription) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcAttributeDescription> select(int maxResult) {
        final List<OcAttributeDescription> result = dao.select(maxResult);
        logger.info("select(OcAttributeDescription) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcAttributeDescription> selectAll() {
        final List<OcAttributeDescription> results = dao.selectAll();
        logger.info("selectAll(OcAttributeDescription) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcAttributeDescription create(OcAttributeDescription bean) {
        requireNonNull(bean);
        logger.info("create(OcAttributeDescription={}) - entered bean ");

        final OcAttributeDescription result = dao.create(bean);

        logger.info("create(OcAttributeDescription) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcAttributeDescription update(OcAttributeDescription bean) {
        requireNonNull(bean);
        logger.info("update(OcAttributeDescription={}) - entered bean ");

        final OcAttributeDescription result = dao.update(bean);

        logger.info("update(OcAttributeDescription) - exited - return value={} result ");
        return result;
    }
}
