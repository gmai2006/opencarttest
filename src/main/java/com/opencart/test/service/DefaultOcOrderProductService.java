/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcOrderProductDao;
import com.opencart.test.entity.OcOrderProduct;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcOrderProductService")
public class DefaultOcOrderProductService implements OcOrderProductService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcOrderProductDao dao;

    @Inject
    @Named("DefaultOcOrderProductDao")
    public DefaultOcOrderProductService(final OcOrderProductDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcOrderProduct find(java.lang.Integer id) {

        final OcOrderProduct result = dao.find(id);
        logger.info("find(OcOrderProduct) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcOrderProduct> select(int maxResult) {
        final List<OcOrderProduct> result = dao.select(maxResult);
        logger.info("select(OcOrderProduct) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcOrderProduct> selectAll() {
        final List<OcOrderProduct> results = dao.selectAll();
        logger.info("selectAll(OcOrderProduct) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcOrderProduct create(OcOrderProduct bean) {
        requireNonNull(bean);
        logger.info("create(OcOrderProduct={}) - entered bean ");

        final OcOrderProduct result = dao.create(bean);

        logger.info("create(OcOrderProduct) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcOrderProduct update(OcOrderProduct bean) {
        requireNonNull(bean);
        logger.info("update(OcOrderProduct={}) - entered bean ");

        final OcOrderProduct result = dao.update(bean);

        logger.info("update(OcOrderProduct) - exited - return value={} result ");
        return result;
    }
}
