/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.opencart.test.service;

import static java.util.Objects.requireNonNull;

import com.opencart.test.dao.OcUserGroupDao;
import com.opencart.test.entity.OcUserGroup;
import java.util.List;
import java.util.logging.Logger;
import javax.ejb.Stateless;
import javax.inject.Inject;
import javax.inject.Named;

@Stateless
@Named("DefaultOcUserGroupService")
public class DefaultOcUserGroupService implements OcUserGroupService {
    private final Logger logger = Logger.getLogger(this.getClass().getName());
    private final OcUserGroupDao dao;

    @Inject
    @Named("DefaultOcUserGroupDao")
    public DefaultOcUserGroupService(final OcUserGroupDao dao) {
        requireNonNull(dao);
        this.dao = dao;
    }

    /** {@inheritDoc} */
    @Override
    public OcUserGroup find(java.lang.Integer id) {

        final OcUserGroup result = dao.find(id);
        logger.info("find(OcUserGroup) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    public List<OcUserGroup> select(int maxResult) {
        final List<OcUserGroup> result = dao.select(maxResult);
        logger.info("select(OcUserGroup) - exited - return value={} result ");

        return result;
    }

    /** {@inheritDoc} */
    public List<OcUserGroup> selectAll() {
        final List<OcUserGroup> results = dao.selectAll();
        logger.info("selectAll(OcUserGroup) - exited - return value={} result ");
        return results;
    }

    /** {@inheritDoc} */
    @Override
    public OcUserGroup create(OcUserGroup bean) {
        requireNonNull(bean);
        logger.info("create(OcUserGroup={}) - entered bean ");

        final OcUserGroup result = dao.create(bean);

        logger.info("create(OcUserGroup) - exited - return value={} result ");
        return result;
    }

    /** {@inheritDoc} */
    @Override
    public OcUserGroup update(OcUserGroup bean) {
        requireNonNull(bean);
        logger.info("update(OcUserGroup={}) - entered bean ");

        final OcUserGroup result = dao.update(bean);

        logger.info("update(OcUserGroup) - exited - return value={} result ");
        return result;
    }
}
