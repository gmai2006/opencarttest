...
// @Stateless
@Named("OcZoneToGeoZoneHandler")
public class OcZoneToGeoZoneHandler extends DelimiterFileHandler<OcZoneToGeoZone> {

    @Inject
    @Named("DefaultJpaDao")
    public OcZoneToGeoZoneHandler(final JpaDao dao) {
        super(dao);
    }

    @Override
    protected OcZoneToGeoZone parseLine(List<String> headers, List<String> tokens) {
        OcZoneToGeoZone record = new OcZoneToGeoZone();
        for (int i = 0; i < tokens.size(); i++) {
            switch (headers.get(i)) {
                case "zoneToGeoZoneId":
                    record.setZoneToGeoZoneId(java.lang.Integer.valueOf((tokens.get(i))));
                    break;
                case "countryId":
