...
// @Stateless
@Named("OcLanguageHandler")
public class OcLanguageHandler extends DelimiterFileHandler<OcLanguage> {

    @Inject
    @Named("DefaultJpaDao")
    public OcLanguageHandler(final JpaDao dao) {
        super(dao);
    }

    @Override
    protected OcLanguage parseLine(List<String> headers, List<String> tokens) {
        OcLanguage record = new OcLanguage();
        for (int i = 0; i < tokens.size(); i++) {
            switch (headers.get(i)) {
                ...
                case "directory":
                    record.setDirectory(tokens.get(i));
                    break;
