...
// @Stateless
@Named("OcOrderProductHandler")
public class OcOrderProductHandler extends DelimiterFileHandler<OcOrderProduct> {

    @Inject
    @Named("DefaultJpaDao")
    public OcOrderProductHandler(final JpaDao dao) {
        super(dao);
    }

    @Override
    protected OcOrderProduct parseLine(List<String> headers, List<String> tokens) {
        OcOrderProduct record = new OcOrderProduct();
        for (int i = 0; i < tokens.size(); i++) {
            switch (headers.get(i)) {
                case "orderProductId":
                    record.setOrderProductId(java.lang.Integer.valueOf((tokens.get(i))));
                    break;
                case "orderId":
                ...
                case "tax":
                    record.setTax(new java.math.BigDecimal(tokens.get(i)));
                    break;

                case "reward":
                    record.setReward(java.lang.Integer.valueOf((tokens.get(i))));
                    break;
