...
// @Stateless
@Named("OcLocationHandler")
public class OcLocationHandler extends DelimiterFileHandler<OcLocation> {

    @Inject
    @Named("DefaultJpaDao")
    public OcLocationHandler(final JpaDao dao) {
        super(dao);
    }

    @Override
    protected OcLocation parseLine(List<String> headers, List<String> tokens) {
        OcLocation record = new OcLocation();
        for (int i = 0; i < tokens.size(); i++) {
            switch (headers.get(i)) {
                ...
                case "geocode":
                    record.setGeocode(tokens.get(i));
                    break;
