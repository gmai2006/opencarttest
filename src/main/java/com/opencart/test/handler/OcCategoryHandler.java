...
// @Stateless
@Named("OcCategoryHandler")
public class OcCategoryHandler extends DelimiterFileHandler<OcCategory> {

    @Inject
    @Named("DefaultJpaDao")
    public OcCategoryHandler(final JpaDao dao) {
        super(dao);
    }

    @Override
    protected OcCategory parseLine(List<String> headers, List<String> tokens) {
        OcCategory record = new OcCategory();
        for (int i = 0; i < tokens.size(); i++) {
            switch (headers.get(i)) {
                ...
                case "parentId":
                    ...
                    break;
                case "top":
                    record.setTop(java.lang.Boolean.valueOf((tokens.get(i))));
                    break;
                case "column":
                    record.setColumn(java.lang.Integer.valueOf((tokens.get(i))));
                    break;
                case "sortOrder":
