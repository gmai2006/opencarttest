...
// @Stateless
@Named("OcProductRecurringHandler")
public class OcProductRecurringHandler extends DelimiterFileHandler<OcProductRecurring> {

    @Inject
    @Named("DefaultJpaDao")
    public OcProductRecurringHandler(final JpaDao dao) {
        super(dao);
    }

    @Override
    protected OcProductRecurring parseLine(List<String> headers, List<String> tokens) {
        OcProductRecurring record = new OcProductRecurring();
