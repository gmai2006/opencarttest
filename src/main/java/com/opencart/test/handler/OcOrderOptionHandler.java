...
// @Stateless
@Named("OcOrderOptionHandler")
public class OcOrderOptionHandler extends DelimiterFileHandler<OcOrderOption> {

    @Inject
    @Named("DefaultJpaDao")
    public OcOrderOptionHandler(final JpaDao dao) {
        super(dao);
    }

    @Override
    protected OcOrderOption parseLine(List<String> headers, List<String> tokens) {
        OcOrderOption record = new OcOrderOption();
        for (int i = 0; i < tokens.size(); i++) {
            switch (headers.get(i)) {
                case "orderOptionId":
                    record.setOrderOptionId(java.lang.Integer.valueOf((tokens.get(i))));
                    break;
                case "orderId":
