...
// @Stateless
@Named("OcCustomerRewardHandler")
public class OcCustomerRewardHandler extends DelimiterFileHandler<OcCustomerReward> {

    @Inject
    @Named("DefaultJpaDao")
    public OcCustomerRewardHandler(final JpaDao dao) {
        super(dao);
    }

    @Override
    protected OcCustomerReward parseLine(List<String> headers, List<String> tokens) {
        OcCustomerReward record = new OcCustomerReward();
        for (int i = 0; i < tokens.size(); i++) {
            switch (headers.get(i)) {
                case "customerRewardId":
                    record.setCustomerRewardId(java.lang.Integer.valueOf((tokens.get(i))));
                    break;
                case "customerId":
