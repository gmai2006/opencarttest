...
// @Stateless
@Named("OcZoneHandler")
public class OcZoneHandler extends DelimiterFileHandler<OcZone> {

    @Inject
    @Named("DefaultJpaDao")
    public OcZoneHandler(final JpaDao dao) {
        super(dao);
    }

    @Override
    protected OcZone parseLine(List<String> headers, List<String> tokens) {
        OcZone record = new OcZone();
