...
// @Stateless
@Named("OcOrderVoucherHandler")
public class OcOrderVoucherHandler extends DelimiterFileHandler<OcOrderVoucher> {

    @Inject
    @Named("DefaultJpaDao")
    public OcOrderVoucherHandler(final JpaDao dao) {
        super(dao);
    }

    @Override
    protected OcOrderVoucher parseLine(List<String> headers, List<String> tokens) {
        OcOrderVoucher record = new OcOrderVoucher();
        for (int i = 0; i < tokens.size(); i++) {
            switch (headers.get(i)) {
                case "orderVoucherId":
                    record.setOrderVoucherId(java.lang.Integer.valueOf((tokens.get(i))));
                    break;
                case "orderId":
