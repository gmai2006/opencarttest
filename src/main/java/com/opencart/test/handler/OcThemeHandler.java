...
// @Stateless
@Named("OcThemeHandler")
public class OcThemeHandler extends DelimiterFileHandler<OcTheme> {

    @Inject
    @Named("DefaultJpaDao")
    public OcThemeHandler(final JpaDao dao) {
        super(dao);
    }

    @Override
    protected OcTheme parseLine(List<String> headers, List<String> tokens) {
        OcTheme record = new OcTheme();
