...
// @Stateless
@Named("OcEventHandler")
public class OcEventHandler extends DelimiterFileHandler<OcEvent> {

    @Inject
    @Named("DefaultJpaDao")
    public OcEventHandler(final JpaDao dao) {
        super(dao);
    }

    @Override
    protected OcEvent parseLine(List<String> headers, List<String> tokens) {
        OcEvent record = new OcEvent();
        for (int i = 0; i < tokens.size(); i++) {
            switch (headers.get(i)) {
                ...
                case "trigger":
                    record.setTrigger(tokens.get(i));
                    break;
