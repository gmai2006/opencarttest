...
// @Stateless
@Named("OcApiHandler")
public class OcApiHandler extends DelimiterFileHandler<OcApi> {

    @Inject
    @Named("DefaultJpaDao")
    public OcApiHandler(final JpaDao dao) {
        super(dao);
    }

    @Override
    protected OcApi parseLine(List<String> headers, List<String> tokens) {
        OcApi record = new OcApi();
