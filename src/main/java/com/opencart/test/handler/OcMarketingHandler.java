...
// @Stateless
@Named("OcMarketingHandler")
public class OcMarketingHandler extends DelimiterFileHandler<OcMarketing> {

    @Inject
    @Named("DefaultJpaDao")
    public OcMarketingHandler(final JpaDao dao) {
        super(dao);
    }

    @Override
    protected OcMarketing parseLine(List<String> headers, List<String> tokens) {
        OcMarketing record = new OcMarketing();
        for (int i = 0; i < tokens.size(); i++) {
            switch (headers.get(i)) {
                ...
                case "clicks":
                    record.setClicks(java.lang.Integer.valueOf((tokens.get(i))));
                    break;
                case "dateAdded":
