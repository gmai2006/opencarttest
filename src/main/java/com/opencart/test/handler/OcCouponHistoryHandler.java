...
// @Stateless
@Named("OcCouponHistoryHandler")
public class OcCouponHistoryHandler extends DelimiterFileHandler<OcCouponHistory> {

    @Inject
    @Named("DefaultJpaDao")
    public OcCouponHistoryHandler(final JpaDao dao) {
        super(dao);
    }

    @Override
    protected OcCouponHistory parseLine(List<String> headers, List<String> tokens) {
        OcCouponHistory record = new OcCouponHistory();
        for (int i = 0; i < tokens.size(); i++) {
            switch (headers.get(i)) {
                case "couponHistoryId":
                    record.setCouponHistoryId(java.lang.Integer.valueOf((tokens.get(i))));
                    break;
                case "couponId":
