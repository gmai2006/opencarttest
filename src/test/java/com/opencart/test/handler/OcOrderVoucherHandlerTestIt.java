...
public class OcOrderVoucherHandlerTestIt {
    static final String inputFile = "OcOrderVoucher.json";
    static OcOrderVoucherHandler handler;
    ...
    static Gson gson =
            new GsonBuilder()
                    ...
                    .create();
    private OcOrderVoucher[] records;
    ...
    public void testSelect() throws IOException {
        ...
        records = gson.fromJson(json, OcOrderVoucher[].class);
        assertEquals("match count", count, records.length);
        OcOrderVoucher testResult = jpa.find(OcOrderVoucher.class, records[0].getOrderVoucherId());
        org.junit.Assert.assertEquals(
                "expect equals orderId ", this.records[0].getOrderId(), testResult.getOrderId());
        org.junit.Assert.assertEquals(
                "expect equals voucherId ",
                this.records[0].getVoucherId(),
                testResult.getVoucherId());
        assertEquals(
        ...
        assertEquals(
                "expect equals fromName ", this.records[0].getFromName(), testResult.getFromName());
        assertEquals(
                "expect equals fromEmail ",
                this.records[0].getFromEmail(),
                testResult.getFromEmail());
        assertEquals("expect equals toName ", this.records[0].getToName(), testResult.getToName());
        assertEquals(
                "expect equals toEmail ", this.records[0].getToEmail(), testResult.getToEmail());
        org.junit.Assert.assertEquals(
                "expect equals voucherThemeId ",
                this.records[0].getVoucherThemeId(),
                testResult.getVoucherThemeId());
        assertEquals(
