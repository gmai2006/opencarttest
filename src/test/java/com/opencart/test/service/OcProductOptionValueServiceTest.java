...
public class OcProductOptionValueServiceTest {
    private static DefaultOcProductOptionValueService serviceMock;
    private static OcProductOptionValue[] records;
    static Gson gson =
            new GsonBuilder()
    ...
    public static void setUp() {
        serviceMock = mock(DefaultOcProductOptionValueService.class);
        String inputFile = "OcProductOptionValue.json";
        try {
            String json =
                    FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
            records = gson.fromJson(json, OcProductOptionValue[].class);
        } catch (IOException ex) {
            ex.printStackTrace();
        }

        // test data
        when(serviceMock.find(records[0].getProductOptionValueId())).thenReturn(records[0]);
    }

    @Test
    public void testFind_success() {
        OcProductOptionValue testResult = serviceMock.find(records[0].getProductOptionValueId());

        org.junit.Assert.assertNotNull(testResult);
        org.junit.Assert.assertTrue(
                "expect equals productOptionId ",
                records[0].getProductOptionId() == testResult.getProductOptionId());
        org.junit.Assert.assertTrue(
        ...
        org.junit.Assert.assertTrue(
                ...
                records[0].getOptionValueId() == testResult.getOptionValueId());
        org.junit.Assert.assertTrue(
