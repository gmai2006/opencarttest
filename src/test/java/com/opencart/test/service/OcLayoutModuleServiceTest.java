...
public class OcLayoutModuleServiceTest {
    private static DefaultOcLayoutModuleService serviceMock;
    private static OcLayoutModule[] records;
    static Gson gson =
            new GsonBuilder()
    ...
    public static void setUp() {
        serviceMock = mock(DefaultOcLayoutModuleService.class);
        String inputFile = "OcLayoutModule.json";
        try {
            String json =
                    FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
            records = gson.fromJson(json, OcLayoutModule[].class);
        } catch (IOException ex) {
            ex.printStackTrace();
        }

        // test data
        when(serviceMock.find(records[0].getLayoutModuleId())).thenReturn(records[0]);
    }

    @Test
    public void testFind_success() {
        OcLayoutModule testResult = serviceMock.find(records[0].getLayoutModuleId());
