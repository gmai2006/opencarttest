...
public class OcZoneServiceTest {
    private static DefaultOcZoneService serviceMock;
    private static OcZone[] records;
    static Gson gson =
            new GsonBuilder()
    ...
    public static void setUp() {
        serviceMock = mock(DefaultOcZoneService.class);
        String inputFile = "OcZone.json";
        try {
            String json =
                    FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
            records = gson.fromJson(json, OcZone[].class);
        } catch (IOException ex) {
            ex.printStackTrace();
        }

        // test data
        when(serviceMock.find(records[0].getZoneId())).thenReturn(records[0]);
    }

    @Test
    public void testFind_success() {
        OcZone testResult = serviceMock.find(records[0].getZoneId());
