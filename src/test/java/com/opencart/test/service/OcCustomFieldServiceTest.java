...
public class OcCustomFieldServiceTest {
    private static DefaultOcCustomFieldService serviceMock;
    private static OcCustomField[] records;
    static Gson gson =
            new GsonBuilder()
    ...
    public static void setUp() {
        serviceMock = mock(DefaultOcCustomFieldService.class);
        String inputFile = "OcCustomField.json";
        try {
            String json =
                    FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
            records = gson.fromJson(json, OcCustomField[].class);
        } catch (IOException ex) {
            ex.printStackTrace();
        }

        // test data
        when(serviceMock.find(records[0].getCustomFieldId())).thenReturn(records[0]);
    }

    @Test
    public void testFind_success() {
        OcCustomField testResult = serviceMock.find(records[0].getCustomFieldId());
        ...
        org.junit.Assert.assertEquals(
                ...
                records[0].getValidation(),
                testResult.getValidation());
        org.junit.Assert.assertEquals(
