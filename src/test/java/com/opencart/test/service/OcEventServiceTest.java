...
public class OcEventServiceTest {
    private static DefaultOcEventService serviceMock;
    private static OcEvent[] records;
    static Gson gson =
            new GsonBuilder()
    ...
    public static void setUp() {
        serviceMock = mock(DefaultOcEventService.class);
        String inputFile = "OcEvent.json";
        try {
            String json =
                    FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
            records = gson.fromJson(json, OcEvent[].class);
    ...
    public void testFind_success() {
        OcEvent testResult = serviceMock.find(records[0].getEventId());

        org.junit.Assert.assertNotNull(testResult);
        org.junit.Assert.assertEquals(
                "expect equals code ", records[0].getCode(), testResult.getCode());
        org.junit.Assert.assertEquals(
                "expect equals trigger ", records[0].getTrigger(), testResult.getTrigger());
        org.junit.Assert.assertEquals(
