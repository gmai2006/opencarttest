...
public class OcProductSpecialServiceTest {
    private static DefaultOcProductSpecialService serviceMock;
    private static OcProductSpecial[] records;
    static Gson gson =
            new GsonBuilder()
    ...
    public static void setUp() {
        serviceMock = mock(DefaultOcProductSpecialService.class);
        String inputFile = "OcProductSpecial.json";
        try {
            String json =
                    FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
            records = gson.fromJson(json, OcProductSpecial[].class);
        } catch (IOException ex) {
            ex.printStackTrace();
        }

        // test data
        when(serviceMock.find(records[0].getProductSpecialId())).thenReturn(records[0]);
    }

    @Test
    public void testFind_success() {
        OcProductSpecial testResult = serviceMock.find(records[0].getProductSpecialId());
