...
public class OcProductDiscountServiceTest {
    private static DefaultOcProductDiscountService serviceMock;
    private static OcProductDiscount[] records;
    static Gson gson =
            new GsonBuilder()
    ...
    public static void setUp() {
        serviceMock = mock(DefaultOcProductDiscountService.class);
        String inputFile = "OcProductDiscount.json";
        try {
            String json =
                    FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
            records = gson.fromJson(json, OcProductDiscount[].class);
        } catch (IOException ex) {
            ex.printStackTrace();
        }

        // test data
        when(serviceMock.find(records[0].getProductDiscountId())).thenReturn(records[0]);
    }

    @Test
    public void testFind_success() {
        OcProductDiscount testResult = serviceMock.find(records[0].getProductDiscountId());
