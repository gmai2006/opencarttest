...
public class OcThemeDaoTestIt {
    static final String inputFile = "OcTheme.json";
    static OcThemeDao dao;
    static Gson gson =
            new GsonBuilder()
    ...
    public void testSelect() {
        OcTheme testResult = dao.find(records[1].getThemeId());
        assertNotNull("expect result", testResult);
        org.junit.Assert.assertTrue(
                "expect equals storeId ", this.records[1].getStoreId() == testResult.getStoreId());
        org.junit.Assert.assertEquals(
                "expect equals theme ", this.records[1].getTheme(), testResult.getTheme());
        org.junit.Assert.assertEquals(
