...
public class OcApiSessionDaoTestIt {
    static final String inputFile = "OcApiSession.json";
    static OcApiSessionDao dao;
    static Gson gson =
            new GsonBuilder()
                    .registerTypeHierarchyAdapter(byte[].class, new ByteArrayToBase64TypeAdapter())
                    .setDateFormat("yyyy-MM-dd HH:mm:ss.S")
                    .create();
    private OcApiSession[] records;
    ...
    public static void beforeClass() {
        ...
        dao = new DefaultOcApiSessionDao(jpa);
    }
    ...
    public void testSelect() {
        OcApiSession testResult = dao.find(records[1].getApiSessionId());
        assertNotNull("expect result", testResult);
        org.junit.Assert.assertTrue(
                "expect equals apiId ", this.records[1].getApiId() == testResult.getApiId());
        org.junit.Assert.assertEquals(
                "expect equals token ", this.records[1].getToken(), testResult.getToken());
        org.junit.Assert.assertEquals(
                "expect equals sessionId ",
                this.records[1].getSessionId(),
                testResult.getSessionId());
        org.junit.Assert.assertEquals(
                "expect equals sessionName ",
                this.records[1].getSessionName(),
